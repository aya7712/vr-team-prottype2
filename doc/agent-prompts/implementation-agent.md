...
**Issue本文だけでなく、コメント欄も必ず確認してください**（`gh issue view
{{ISSUE_URL}} --comments` 等）。コメントに実装方針への追加指示・訂正・除外条件が
書かれている場合、案の説明より新しい可能性があるため優先して従ってください
（案の再解釈はしないという原則の範囲内で、Issueコメントの追加指示は反映すること）。
