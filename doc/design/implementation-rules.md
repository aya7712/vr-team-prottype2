...
### 9.1 自己レビュー実施のpre-commit検証

`doc/todo.md`の運用ルールでは、実装後に必ずサブエージェント（Sonnet固定）へ自己レビューを行わせ、コミットメッセージに以下のtrailerを残すことにしている。

```
Self-Review: <model>, TODO=<TODO番号>, findings=<件数-対応状況>
```

このtrailerの付け忘れ（＝自己レビューを飛ばして直接コミットする）を機械的に防ぐため、Huskyの`pre-commit`（または`commit-msg`）フックを設け、以下を検証する。

- `packages/`配下（実装コード）の変更を含むコミットは、コミットメッセージに上記trailerが含まれていなければコミットを拒否する。
- `doc/`のみの変更（設計ドキュメント更新等）はこの検証の対象外とする。
- このチェックはあくまで「trailerが存在するか」という形式的な検証であり、**レビューの中身の妥当性までは保証しない**。実際にレビューを実施したかどうかは運用（`doc/todo.md`の手順順守）に依存する点に留意する。
