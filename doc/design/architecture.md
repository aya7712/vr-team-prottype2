...
| `POST /api/sessions` | 新規会話セッション作成（参加キャラクター2〜4体、最初のトピック（必須、T35）、シナリオ設定を受付：F6.6） |
| `GET /api/sessions` | セッション一覧取得（作成日時降順）。ログ閲覧画面（F9.4）のセッション選択に使う（T33で追加） |
...
UIはWebSocket接続時に直近セッションのイベントをそのまま購読し、F9.1〜F9.3をリアルタイム更新する。F9.4（ログ閲覧）はRESTのターン取得APIを用いて過去ログを表示し、同じUIコンポーネントで再描画する。`session:end`はターン単位のレイヤーイベントとは異なり、`maxTurns`分の生成完了・`POST /:id/stop`による打ち切り・LLM/Embedding呼び出し例外による中断のいずれかでセッションの実行ループが終了した際に一度だけ発行される（T40/T41）。
...
- リトライ・タイムアウトはプロトタイプのため最小限とする（1回リトライ、180秒タイムアウト、リトライ前5秒待機。当初10秒タイムアウト・待機なしとしていたが、T19のE2E確認で一部モデル（例: google/gemma-4-31B-it）の応答が14〜38秒程度と不安定・低速であること、およびTogether AI側の一時的な500エラーが連続し待機なしのリトライでは回復を待てないことが判明したため、60秒タイムアウト・5秒待機に変更した。さらにtone_sample追加後のE2E確認で、google/gemma-4-31B-it（reasoning出力を伴うdenseモデルで約35 tokens/秒と構造的に低速）が本番相当の長いプロンプトで60秒を安定して超過することが判明したため、180秒タイムアウトに再変更した）。
