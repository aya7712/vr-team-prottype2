...
- [x] **T43. 話者自身の過去発話をmemory層で永続化し、意味検索でvoice-exemplarとして提示する（Issue #5, plan-f）**
  2026-09-01、Issue #5「キャラクターの口調が間違うことがあります。特に、前に発言したキャラクターの発言に引っ張られて口調がずれているように感じます」への対応として、複数案のうちの1案（plan-f）として着手。他の案（プロンプトテンプレート層での模倣禁止の注意書き、character層での自然言語の口調プロファイル提示、生成後の口調逸脱検知・再生成）とは異なり、memory層（F3）と埋め込み検索インフラを使う。
  `ConversationManager.runTurn`の発話生成後に、生成された発話をそのキャラクター自身の「自己記憶」（`MemoryItem`、`source: 'session'`、`participants`は話者のみ、`shareable: false`）として`MemoryRepository`（`session_memories`, data-design.md D8）に永続化する処理を追加した。次のターンでそのキャラクターが再び発話する際、`MemoryRetriever`に新設した`retrieveSelfVoiceExemplars`が既存の`EmbeddingService`（Together AI Embeddings API）による意味検索で現在の話題に近い自分の過去発話を最大2件取得し、`prompts/utterance/base.md`の新セクション「（参考）あなた自身の過去の発言例」として`toneSample`と並べて提示する。
  **実施内容（対応済み）**: `MemoryRepository`インターフェースに`saveSessionMemory`/`saveEmbedding`（後者はT15でMemoryRepositoryImpl固有のwrite専用メソッドだったものをinterfaceへ昇格）を追加し、`InMemoryMemoryRepository`（テスト用フェイク）・`MemoryRepositoryImpl`（SQLite実装）双方で実装した。`MemoryRetriever`に`recordSelfUtterance`（保存）と`retrieveSelfVoiceExemplars`（想起）を追加。既存の`retrieve()`のプール（`MAX_RESULTS=3`、shareable:falseは会話相手がいる場合に除外）とは別枠（`MAX_SELF_VOICE_RESULTS=2`、shareableフィルタなし）にすることで、リスクとして懸念していた「自己発話由来の記憶がプリセット記憶の想起結果を埋没させる」問題を回避した（data-design.md 6.3に理由を明記）。`EmbeddingClient`/`EmbeddingService`に`getModel()`を追加し、`memory_embeddings.model`列へ書き込むモデル名をMemoryRetriever側で重複管理しないようにした。`ConversationManager.runTurn`で`retrieve`と`retrieveSelfVoiceExemplars`を並行実行し、`layer:memory`イベントへ`selfVoiceExemplars`を追加、発話生成後に`recordSelfUtterance`を呼ぶよう配線した。`exportConversationReport.ts`のレポート表示にも反映した。
  テスト: `MemoryRetriever.test.ts`/`InMemoryMemoryRepository.test.ts`/`MemoryRepositoryImpl.test.ts`/`EmbeddingClient.test.ts`/`EmbeddingService.test.ts`に新規ユニットテストを追加（自己記憶の保存・自己記憶のみを対象にした想起・shareable:falseでも除外されないこと・embeddingServiceありの意味的ランキング等）。`ConversationManager.test.ts`に、発話後に自己記憶が保存されること、2巡目以降のプロンプトに`{{selfVoiceExemplars}}`として反映されることを確認する統合的なテストを追加した。実運用確認として`e2eConversation.js`で実セッション（20ターン基本）を生成し、`exportConversationReport.js`のHTMLレポートで`layer:memory`イベントに`selfVoiceExemplars`が想起されていることを目視確認した。
  **自己レビューで発見・対応した不具合**: `/code-review`スキルによる独立レビューで、`retrieveSelfVoiceExemplars`が当初`getAllCandidates`（`session_memories`をセッション横断で返す）を流用していたため、別セッションで生成された自己発話まで「このセッション中の発言例」として混入しうる不具合が見つかった。`MemoryRepository`に`getSelfVoiceCandidates(sessionId, speakerId)`（SQL側で`session_id`と`owner`を絞り込む専用メソッド）を追加し修正、回帰防止テストを`MemoryRetriever.test.ts`/`MemoryRepositoryImpl.test.ts`双方に追加した（data-design.md 6.3に詳細を記載）。もう1件、`packages/ui`のリアルタイム画面（`LayerInspector`/`LogBrowser`）が`selfVoiceExemplars`の表示に未対応という指摘もあったが、今回のtargetLayersの対象外（`packages/ui`は非対象）と判断し見送った。オフライン確認用の`exportConversationReport.ts`（対象範囲内）では表示に対応済み。
