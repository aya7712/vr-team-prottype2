...
- [ ] **T03a. 自己レビューtrailer検証用pre-commitフックの導入**
  `implementation-rules.md` 9章に従い、Huskyの`pre-commit`（または`commit-msg`）フックを導入し、`packages/`配下の変更を含むコミットで、コミットメッセージに `Self-Review: <model>, TODO=<番号>, findings=<内容>` のtrailerが含まれない場合はコミットを拒否するチェックスクリプトを実装する。`doc/`のみの変更（設計ドキュメント更新等）はチェック対象外とする。
  テスト: trailer無しでのコミットが拒否されること、trailer有りでのコミットが成功すること、`doc/`のみの変更ではtrailer無しでも成功することを確認する（`git commit`をサンドボックスの一時リポジトリ等で試すユニット/スクリプトテストとして書く）。
